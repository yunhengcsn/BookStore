...
<script type="text/javascript">
	function _go() {
		...
		location = "${pageBean.url}&currPage=" + pc;
	}
</script>
  ...
  <div class="divContent">
    <%--上一页 --%>
        <c:choose>
            ...
            <c:otherwise>
                <a href="${pageBean.url}&currPage=${pageBean.currPage-1}" class="aBtn bold">上一页</a>
            </c:otherwise>
        </c:choose>
    <%-- 计算begin和end --%>
        <c:choose>
            ...
            <c:otherwise>
                ...
                <c:set var="begin" value="${pageBean.currPage-2}"/>
                <c:set var="end" value="${pageBean.currPage+3}"/>
    ...
    <%-- 显示页码列表 --%>
        <c:forEach begin="${begin}" end="${end}" var="i">
            <c:choose>
                <c:when test="${pageBean.currPage eq i}">
                    <span class="spanBtnSelect">${i}</span>
                </c:when>
                <c:otherwise>
                    <a href="${pageBean.url}&currPage=${i}" class="aBtn">${i}</a>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    ...
    <%--下一页 --%>
        <c:choose>
            <c:when test="${pageBean.currPage eq pageBean.totalPages || pageBean.totalPages eq 0}">
                <span class="spanBtnDisabled">下一页</span>
            </c:when>
            <c:otherwise>
                <a href="${pageBean.url}&currPage=${pageBean.currPage+1}" class="aBtn bold">下一页</a>
            </c:otherwise>
        </c:choose>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
